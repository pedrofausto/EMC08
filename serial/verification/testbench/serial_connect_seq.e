...
// -----------------------------------------------------------------------------
// File Name : serial_connect_seq.e
...
extend ACTIVE SERIAL_IN serial_agent_u {
   // driver isntance to generate the input packet
   driver: serial_driver_u is instance;
...
extend serial_driver_u {
   // pointer to the agent
   agent: serial_agent_u;
...
extend serial_bfm_i_u {	
   ...
   driver: serial_driver_u; 
  
   // method that get a packet from driver
   get_pkt(): serial_signal_type @clock_e is also {
