...
extend top_mon_u {
   ...
   !p_top_baud_rate: inout simple_port of bit;
   !p_top_serial_tx: inout simple_port of bit;
   ...
   event baud_rate_rise is rise(p_top_baud_rate$)@sim;
   
   event serial_tx_rise is rise(p_top_serial_tx$)@sim;
      ...
      watch_bus() @clock_e is also {
         while TRUE {   
            ...
            case sys.feature {
               ...
               feature_behave: {functional_model_behave();};
   ...
   check_a_p3_0 (data: bit) is {
      if (p_top_p3_a_o$[0:0] != data) { out ("Data a_p3_0 ERRORRRRR - expected ---> ", data);
                                        out ("                      - received ---> ", p_top_p3_a_o$[0:0])};
      check that (p_top_p3_a_o$[0:0] == data);
   }; //end checker
   
   check_a_p3_1 (data: bit) is {
      if (p_top_p3_a_o$[1:1] != data) { out ("Data a_p3_1 ERRORRRRR - expected ---> ", data);
                                        out ("                      - received ---> ", p_top_p3_a_o$[1:1])};
      check that (p_top_p3_a_o$[1:1] == data);
   }; //end checker
