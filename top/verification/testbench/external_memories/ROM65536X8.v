...
primitive ROM65536X8_DFF  (Q, D, C, RN, SN, NOTIFY);
...
module ROM65536X8 (Q, ADR, CLK, ENB, OEB, romvdd, romgnd);
...
ROM65536X8_DFF CHK_FF (Q_OK, supply_OK, CLK_, 1'b1, 1'b1, NOTIFY_REG);
...
`else
initial	for(i = 0; i < 65536; i = i + 1) ROM_matrix[i] = ~8'b0;
always @(posedge CLK) if(enableMem)
begin
	if(ADR_ < 65536) QR = ROM_matrix[ADR_];
...
//initial $readmemb("ROM65536X8.bin", ROM_matrix);
initial $readmemh("ROM65536X8.vhe", ROM_matrix);
//*****************************************************************************************
