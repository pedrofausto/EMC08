...
extend sys {serial_tx: bit};

extend sys {opcode: uint(bits:8)};

extend sys {op1:uint(bits:8)};
extend sys {op2:uint(bits:8)};
extend sys {op3:uint(bits:8)};

extend sys {data_rx:uint(bits:8)};
extend sys {data_tx:uint(bits:8)};

type feature_kind_t: [feature_reset,feature_ports,  feature_timers, feature_mem_ram, feature_serial, feature_interrupt,  feature_core, feature_behave];
