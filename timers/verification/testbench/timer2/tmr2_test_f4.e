...
extend tmr2_mon_u {
  ...
  keep sel_feat == FEAT_4        ;
  
  cover cover_func_model is  also {
  
     item feature_f4_cov : feature_f4_t = feature_f4 using illegal = (feature_f4_cov == EDGE_ERROR);
...
extend MAIN tmr2_seq_s {
   ...
   !seq_acr_inc         : SEQ_ACR_INC     tmr2_seq_s;
   
   !num_pkt             : byte;
   
   keep soft num_pkt in [1..1] ;
...
//        };
   ...
   body() @driver.clock is only {
      ...
      out ("TEST DESCRIPTION = Edge Detector \t FEATURE NUMBER = 04  STARTS HERE ...\n");
